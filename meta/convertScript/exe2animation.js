...
for (let i = 0; i < exeByteSize; i++) {
    if (context == 'detectingTag') {
        ...
        if (fileNameCounter < 0) {
            ...
            if (fileNameBuffer.substring(fileNameBuffer.length - 4) == '.pcx' 
             || fileNameBuffer.substring(fileNameBuffer.length - 4) == '.PCX') {
                outputStr += '0x' + i.toString(16).toUpperCase().padStart(6, '0') + ' : ' + fileNameBuffer.padEnd(15)  // 주소 + 파일명 출력
        ...
        if(detectedTag == '@'){ // ase_ps, 가변 프레임 크기임. 애니메이션 번호?, 프레임의 위치, 크기, offset 정보, 4바이트씩 총 7개
            for(let j = 16 ; j < 64 ; j += 4){  // .pcx 가 나온 이후 16바이트는 무조건 0이어서 생략함. (0x1CED68 : @Gatewarp.pcx는 데이터가 있긴 한데 의미불명)
            ...
            for(let j = 16 ; j < 32 ; j += 2){  // .pcx 가 나온 이후 16바이트는 무조건 0이어서 생략함. 
